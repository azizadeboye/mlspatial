---
title: "Spatial Machine Learning workflow"
...
oldpar <- par(mfrow = c(1, 1)) # Reset layout
on.exit(par(oldpar))
...
# Save to PDF
ggsave(file.path(tempdir(), "Observed_vs_Incidence_All_Models.pdf", plot = combined_plot, width = 12, height = 4))

# Save to PNG
ggsave(file.path(tempdir(), "Observed_vs_Predicted_All_Models.png", plot = combined_plot, width = 12, height = 4, dpi = 300))
```
```{r}
## CROSS-VALIDATION
...
# export
write.csv(residuals_df, file.path(tempdir(), "model_residuals.csv", row.names = FALSE))
library(writexl)
write_xlsx(residuals_df, file.path(tempdir(), "model_residuals.xlsx"))
