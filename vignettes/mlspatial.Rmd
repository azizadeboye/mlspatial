...
Cross-validation is a critical step for evaluating model performance and ensuring robust, generalizable predictions. The `mlspatial` workflow supports multiple algorithms, including Random Forest (RF), XGBoost (XGB), and Support Vector Regression (SVR).
...
- Quantify model errors across the dataset (via boxplots).
...
- Barplots for overall RMSE, MAE, and R² across models.
- Spatial maps showing residuals, annotated with performance metrics, to visually identify regions with higher prediction errors.
